...
import { CharacterPosition, EnemyPosition } from "../../data-types/characterType";
...
const CharacterSheet: React.FC<CharacterSheetProps> = ({ setIsCharSheetShown, dBPositions, isCharSheetShown, enemyPositions }) => {
    ...
    <>
      ...
      <>
        {(loading || !characterAttributes || !isCharSheetShown) ?
          ...
          :
          <dialog open className="modal rounded-md">
            <div className="bg-gray-900 rounded-md w-5/12 min-w-40">
              <div className='flex justify-between pl-4 pt-2 pb-2 text-yellow-500'>
                <h2 className="font-bold text-2xl">{`${dBPositions && dBPositions[0].charId}`}</h2>
                <button className=" hover:text-3xl hover:text-gray-300 transform transition-all flex w-10 h-10 bg-transparent justify-center items-center text-2xl font-extrabold cursor-pointer mr-1" data-dialog-close="true" type="button" onClick={() => setIsCharSheetShown(false)}>X</button>
              </div>
              <div className="flex justify-evenly bg-white ml-4 mr-4 rounded-md">
                ...
                <div className='h-72 w-72 flex flex-col justify-evenly'>
                  <div className='flex flex-row items-center border-b-2 text-gray-900'>
                  ...
                  <div className='flex flex-row w-auto justify-between items-center border-b-2 text-gray-900'>
